  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
        ...
        CREATE TABLE mentor_tools(
            ...
            CONSTRAINT fk_mentor_tool FOREIGN KEY (mentor_id) REFERENCES mentor(id) ON DELETE CASCADE,
            PRIMARY KEY(id),
            UNIQUE(mentor_id, tool)
        );
    
        COMMENT ON TABLE mentor_tools IS 'The mentor selected tools for the profile';
        COMMENT ON TYPE TOOLS IS 'All options of tools, used in the tool column type';
