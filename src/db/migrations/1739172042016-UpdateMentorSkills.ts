...
export class UpdateMentorSkills1739172042016 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    queryRunner.query(`
            ...
            CREATE TABLE mentor_skills(
                ...
                CONSTRAINT fk_mentor_skills FOREIGN KEY (mentor_id) REFERENCES mentor(id) ON DELETE CASCADE,
                PRIMARY KEY(id),
                UNIQUE(mentor_id, skills)
            );

            COMMENT ON TABLE mentor_skills IS 'The mentor selected skills for the profile';
            COMMENT ON TYPE SKILLS IS 'All options of skills, used in the skills column type';
            COMMENT ON COLUMN mentor_skills.mentor_id IS 'The UUID of the mentor associated with a specific skill'; 
            COMMENT ON COLUMN mentor_skills.skills IS 'The skill that the mentor specializes in, using the SKILLS enum to ensure valid values';
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    queryRunner.query(`
            ...
            ALTER TABLE mentor ADD COLUMN skills JSONB NOT NULL;
    `)
  }
}
